...
- 在对搜索引擎敏感的应用这个是必须的，特别是针对个人站点、资讯站点内容需要被搜索引擎索引到才有更大的流量。
...
- 第 2 种场景就是需要很快的首屏加载的应用。
- 更通用的解决方案
  - Nuxt： vue 生态推荐的 SSR 解决方案
  - Quasar：一个基于 Vue 的完整解决方案
  - [Next.js](https://nextjs.org/showcase)： react 框架的 SSR 方案
...
代码的逻辑复用以现代编程语言必须具备的功能，vue 在逻辑复用技巧

- 组合式函数：逻辑复用
- 自定义指令：
- 插件： 一种能为 Vue 增加全局功能的工具代码。类似于 java 中的 Jar 包的概念，或者 C++的 so 的概念，这样使得程序具备了大规模协作的可能。
